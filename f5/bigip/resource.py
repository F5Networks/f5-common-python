...
class StatsBase(PathElement, ToDictMixin):
    ...
    """
    def __init__(self, container):
        super(StatsBase, self).__init__(container)
    ...
    @staticmethod
    def _parse_keys(rdict):
        """Pop 'entries' and use stat names as attributes"""
        temp_dict = rdict
        stat_vals = rdict.pop('entries')
        temp_dict.update(stat_vals)
        return temp_dict
    ...
    def _local_update(self, rdict):
        ...
        """
        sanitized = self._check_keys(rdict)
        dot_replace = self._key_dot_replace(sanitized)
        temp_meta = self._meta_data
        self.__dict__ = self._parse_keys(dot_replace)
        self._meta_data = temp_meta
    ...
    def create(self, **kwargs):
        '''Create is not supported for Stats resource
    ...
    def delete(self, **kwargs):
        '''Delete is not supported for Stats resource
